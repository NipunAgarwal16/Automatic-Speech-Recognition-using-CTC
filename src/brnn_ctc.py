...
def create_batches(data, label, max_seq_length, batch_size, rand_idx, mode="train"):
    ...
    for i in xrange(num_batches):
        ...
        for j, idx in enumerate(batch_idx):
            x = data[idx]
            data_in_batch[0 : x.shape[1], j, :] = np.reshape(
                x, (x.shape[1], num_features)
            )
...
def main():
    ...
    (test_data_tensor, test_seq_lengths), (
        test_label_indices,
        test_label_vals,
        test_label_shape,
    ) = list(
        ...
        0
    ]  # borrow create_batch to transform test data / label
    ...
    cur_tb_summary_path = os.path.join(
        TENSORBOARD_LOG_DIR, "{:.0f}".format(cur_unixtime)
    )
    ...
    with graph.as_default():
        ...
        X_train = tf.placeholder(
            tf.float32, shape=(max_seq_length, batch_size, num_features), name="X_train"
        )
        X_test = tf.placeholder(
            tf.float32,
            shape=(max_seq_length, num_test_samples, num_features),
            name="X_test",
        )
        ...
        y_train = tf.cast(
            tf.SparseTensor(y_train_indices, y_train_vals, y_train_shape), tf.int32
        )
        ...
        y_test = tf.cast(
            tf.SparseTensor(y_test_indices, y_test_vals, y_test_shape), tf.int32
        )
        ...
        def multi_brnn_layer(
            ...
            for n in range(num_layers):
                forward_cell = rnn_cell_fn(
                    num_rnn_hidden, activation=rnn_cell_activation_fn
                )
                backward_cell = rnn_cell_fn(
                    num_rnn_hidden, activation=rnn_cell_activation_fn
                )
                inner_outputs = brnn_layer(
                    forward_cell,
                    backward_cell,
                    inner_outputs,
                    seq_lengths,
                ...
                if use_dropout:
                    inner_outputs = tf.contrib.layers.dropout(
                        inner_outputs, keep_prob=keep_prob, is_training=is_training
                    )

            return inner_outputs

        with tf.variable_scope("rlan") as scope:
            brnn_outputs_train = multi_brnn_layer(
                X_train, seq_lengths_train, num_rnn_layers, True, keep_prob=keep_prob
            )
            brnn_outputs_train = [
            ...
            brnn_outputs_test = multi_brnn_layer(
                X_test, seq_lengths_test, num_rnn_layers, False, keep_prob=keep_prob
            )
            brnn_outputs_test = [
        ...
        fc_W = tf.get_variable(
            "fc_W", initializer=tf.truncated_normal([num_rnn_hidden, num_classes])
        )
        ...
        pred_train = tf.to_int32(
            tf.nn.ctc_beam_search_decoder(
                logits3d_train, seq_lengths_train, merge_repeated=False
            )[0][0]
        )
        ...
        pred_test = tf.to_int32(
            tf.nn.ctc_beam_search_decoder(
                logits3d_test, seq_lengths_test, merge_repeated=False
            )[0][0]
        )
    ...
    with tf.Session(graph=graph, config=config) as sess:
        saver = tf.train.Saver(
            tf.global_variables(), max_to_keep=5, keep_checkpoint_every_n_hours=1
        )
        ...
        for epoch in range(num_epochs):
            ...
            perm = (
                range(num_samples) if epoch == 0 else np.random.permutation(num_samples)
            )
            batches = create_batches(
                train_data, train_label, max_seq_length, batch_size, perm
            )

            for batch, (
                (batch_data, batch_seq_lengths),
                (batch_indices, batch_vals, batch_shape),
                ...
                if epoch > 0 and batch == 0:
                    ...
                    merged_test_per = calc_PER(
                        SparseTensor(
                            cur_test_pred.indices,
                            cur_test_pred.values,
                            cur_test_pred.dense_shape,
                        ),
                        SparseTensor(
                            test_label_indices, test_label_vals, test_label_shape
                        ),
                    )
                    ...
                    print(
                        "[epoch: {}, batch: {}] err_train = {:.4f} (phn_merged: {:.4f}))".format(
                            epoch,
                            batch,
                            batch_err_rate,
                            merged_train_per,
                    ...
                    _, batch_loss, batch_err_rate, batch_pred = sess.run(
                        [optimizer, loss_train, err_rate_train, pred_train],
                        feed_dict={
                            X_train: batch_data,
                            y_train_indices: batch_indices,
                            y_train_vals: batch_vals,
            ...
            saver.save(
                sess, os.path.join(cur_checkpoint_path, "model"), global_step=epoch
            )
